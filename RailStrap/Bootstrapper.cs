...
namespace RailStrap
{
    public class Bootstrapper
    {
        ...
        public string BackgroundUpdaterMutexName => $"RailStrap-BackgroundUpdater-{_launchMode}";

        public string MutexNamePrefix { get; set; } = "RailStrap-Bootstrapper";
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one RailStrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingRailStrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "RailStrap.exe");
